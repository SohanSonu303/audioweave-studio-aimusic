...
import { Play, Pause, Download, ChevronLeft, ChevronRight, Disc3 } from "lucide-react";
import { useWaveSurfer } from "@/hooks/use-wavesurfer";
import { formatTimeWithDecimal } from "@/lib/utils";
import { WaveformTimeline } from "@/components/edit/waveform-timeline";
import type { CoverTrack } from "@/lib/api/cover";

interface CoverResultPlayerProps {
  tracks: CoverTrack[];
}

export function CoverResultPlayer({ tracks }: CoverResultPlayerProps) {
  ...
  const handleDownload = () => {
    ...
    a.download = `${active.title ?? "cover"}.mp3`;
    a.click();
  };
      ...
      >
        <div className="flex items-center gap-4 min-w-0">
          <button
            ...
            className="w-10 h-10 rounded-full flex items-center justify-center transition-all hover:scale-105 active:scale-95 disabled:opacity-30 flex-shrink-0"
          ...
          {/* Album art the backend generated for this cover */}
          {active?.album_cover_path ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={active.album_cover_path}
              alt=""
            ...
            >
              <Disc3 size={16} className="text-aw-text-3" />
            </div>
          )}

          <div className="flex flex-col min-w-0">
            <span className="text-[14px] font-semibold text-aw-text truncate">
              {active?.title ?? "Generated Cover"}
            </span>
